...
const aesEncrypt = async (data, key) => {
    console.log("key", key);
...
const aesDecrypt = async (blob, key) => {
...
export const encryptMetaBlob = async (url, username, key) => {
    const composite = JSON.stringify({ url, username });
    return aesEncrypt(composite, key);
};

export const decryptMetaBlob = async (blob, key) => {
    const decrypted = await aesDecrypt(blob, key);
    return JSON.parse(decrypted);
};

export const encryptPasswordBlob = async (password, key) => {
    return aesEncrypt(password, key);
};

export const decryptPasswordBlob = async (blob, key) => {
